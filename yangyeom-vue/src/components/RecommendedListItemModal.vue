<template>
<div class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" :id="'recom-' + recom.movie.code">
  <div class="modal-dialog modal-xl" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">{{ recom.movie.title }} | 예상평점: {{ recom.score }}점/5.0점</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <img :src="poster_url" :alt="recom.movie.title" width="200px">
      </div>
      <div class="modal-body">
        <hr>
        <p>{{ recom.movie.description }}</p>
      </div>
      <div class="modal-body">
        <hr>
        <div v-if="ifIRated">
...
export default {
  ...
  data() {
      return {
        poster_url: `http://image.tmdb.org/t/p/w500/${this.recom.movie.poster_url}`,
        reviews: [],
        content_rating: '',
        score_rating: 0,
      }
  ...
  methods: {
    save() {
      if (!this.ifIRated) {
        ...
        const data = {
          ...
          user: this.user,
          movie: this.recom.movie.code
        }
        axios.post(`http://127.0.0.1:8000/api/v1/movie/${this.recom.movie.code}/reviews/`, data, this.options)
          .then(response => {
    ...
    reviewDelete(review_id) {
      ...
      conf.user = this.user
      console.log(conf)
      axios.delete(`http://127.0.0.1:8000/api/v1/movie/${this.recom.movie.code}/reviews/${review_id}/`, conf)
        .then(response => {
            console.log(response)
  ...
  mounted(){
    ...
    // console.log('리뷰 가져올 때 conf', conf)
      axios.get(`http://127.0.0.1:8000/api/v1/movie/${this.recom.movie.code}/reviews/`)
        .then(response => {
            this.reviews = response.data
            // this.myReview = this.reviews.filter(review => review.user === this.user)
            // if (this.myReview.length !== 0) {
            //   this.ifIRated = true
            //   console.log(this.recom.movie.title,'에서내가 쓴 리뷰', this.myReview)
            //   }
        })
        .catch(error => {
            console.log(error)
  ...
  computed: {
    ...
    ifIRated() {
      // console.log(this.recom.movie.title, '에서 ifIRated', a)
